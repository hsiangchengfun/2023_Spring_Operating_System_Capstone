...
static struct frame frame_array[(MEM_END-MEM_START)/PAGE_SIZE];     //entries, store memory's statement and page's corresponding index
...
void *malloc(unsigned int size){
    ...
    frame_list[size_level]->prev = NULL;
...
void free(void *addr){
    ...
    // printf("[System Message] Free Done\r\n");
...
void init_mm(){
    n_frames = (MEM_END-MEM_START) / PAGE_SIZE;       //number of frames
...
void chunk_free(void *address) {
    ...
    for (uint32_t i = 0; i < MAX_POOLS; i++) {
        for (uint32_t j = 0; j < pools[i].pages_used; j++) {
...
void memory_reserve(void* start, void* end ,int ind) {
    ...
    reserved_list[ind].pair.fst = start;
    reserved_list[ind].pair.sec = end;
}