...
unsigned long map_table(unsigned long *table, unsigned long shift, unsigned long va, int* new_table) {
    ...
    index = index & (PTRS_PER_TABLE - 1);//take last nine bits
    if (!table[index]) {
        *new_table = 1;
    ...
    // table[] stores  next level table address
    // so and MASK and return it
}
...
void map_table_entry(unsigned long *pte, unsigned long va, unsigned long pa) {
    ...
    pte[index] = entry;
    // point pte table to the entry address
...
unsigned long va2phys(unsigned long va) {
    ...
    unsigned long phys = (((unsigned long*)(pte + VA_START))[va>>PTE_SHIFT & (512-1)]) & PAGE_MASK;
    printf("PGD         PUD         PMD         PTE         Physic\n");
