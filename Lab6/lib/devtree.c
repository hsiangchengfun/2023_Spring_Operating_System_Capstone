...
void fdt_traverse( void (*callback)(char *, char *, struct fdt_prop *) ) {
    ...
    void *dt_struct_addr = DEVTREE_ADDRESS + fdt32_to_cpu(devtree_header->off_dt_struct);
    char *dt_string_addr = DEVTREE_ADDRESS + fdt32_to_cpu(devtree_header->off_dt_strings);
    ...
    while (1) {
        ...
        token = fdt32_to_cpu(*((unsigned int *)dt_struct_addr));
        ...
        if (token == FDT_BEGIN_NODE) {
            ...
            off = 4 + 8 + fdt32_to_cpu(prop->len);
            if (off%4 != 0)
                off = ((off/4)+1)*4;

            dt_struct_addr += off;
            
            prop_name = dt_string_addr + fdt32_to_cpu(prop->nameoff);
