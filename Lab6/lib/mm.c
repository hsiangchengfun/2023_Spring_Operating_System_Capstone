...
void free(void *addr){
    ...
    if(target->state == ALLOCABLE || target->state == C_NALLOCABLE){
        printf("[ERROR] Invalid free address (already freed)\r\n");
        return ;
    }
...
void init_mm_reserve() {
    ...
    memory_reserve((void*)0x0, (void*)((unsigned long long)__kernel_end_ptr&VA_MASK),0); // spin tables, kernel image
    memory_reserve((void*)0x20000000, (void*)DEVTREE_CPIO_END,1); // hard code reserve initramfs
